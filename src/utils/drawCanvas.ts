...
const drawCircle = (ctx: CanvasRenderingContext2D, handLandmarks: NormalizedLandmarkListList) => {
    if (handLandmarks.length > 0 && handLandmarks[0].length > 8) {
        const width = ctx.canvas.width
        const height = ctx.canvas.height
        const [x1, y1, z1] = [handLandmarks[0][8].x * width, handLandmarks[0][8].y * height, handLandmarks[0][8].z * width]
        const [x2, y2, z2] = [handLandmarks[0][4].x * width, handLandmarks[0][4].y * height, handLandmarks[0][4].z * width]
        const x = (x1 + x2) / 2
        const y = (y1 + y2) / 2
        const r = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2) + Math.pow(z1 - z2, 2)) / 2
